...
abstract contract AbstractCradleNativeListing is AbstractContractAuthority, ReentrancyGuard {
    ...
    event ListingPurchase(address indexed user, uint256 amount);

    event ListingReturn(address indexed user, uint256 amount);
    ...
    function withdrawToBeneficiary(uint256 amount) public onlyAuthorized nonReentrant {
    ...
    {
        return (
            total_distributed, max_supply - total_distributed, raised_asset_amount, raised_asset_balance, uint8(status)
        );
    }

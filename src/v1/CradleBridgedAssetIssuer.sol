...
contract CradleBridgedAssetsIssuer {
    ...
    function createBridgedAsset(string memory _name, string memory _symbol) public payable returns (address) {
    ...
    function releaseStable(address user, string memory symbol, uint256 burnAmount, uint256 releaseAmount)
        public
        onlyProtocol
    {
        AbstractCradleAssetManager asset = bridgedAssets[symbol];
